...
test("General Test - Check Data Presence Ethereum Mainnet", async ({
  page,
}) => {
  await page.goto(
  ...
  // dashboard tab
  await expect(
    page.getByRole("button", { name: "ET E2E Test DAO (Ethereum)" })
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByLabel("breadcrumbs").getByText("TOKENVOTING-")
  ).toBeVisible();
  await expect(
