...
test('General Test - Check Data Presence Polygon Mainnet', async ({ page }) => {
    await page.goto(
        '/dao/polygon-mainnet-0x71a363934240841c0a0f467e2fa4187199adb4d3/dashboard', // polygon-mainnet DAO using multisigPlugin
    );
    // dashboard tab
    await expect(page.getByRole('button', { name: 'ET E2E Test DAO (Polygon)' })).toBeVisible();
    ...
    await expect(page.getByRole('link', { name: /avatar USDC \$\d+\.\d{2} 1\.23 USDC/ })).toBeVisible();
    await expect(page.getByRole('link', { name: /Polygon Ecosystem Token \$\d+\.\d{2}/ })).toBeVisible();
    await expect(page.getByRole('link', { name: /avatar SporkDAO \$\d+\.\d{2} 2 SPORK/ })).toBeVisible();
    ...
    await expect(page.getByRole('heading', { name: 'Multisig (MULTISIG)' })).toBeVisible();
    await expect(page.getByRole('tab', { name: 'Contract' })).toBeVisible();
    await expect(page.getByText('Plugin addressMultisig v1.20x1E…')).toBeVisible();
    await expect(page.getByText('LaunchedFebruary')).toBeVisible();
    ...
    await expect(page.getByLabel('breadcrumbs').getByText('MULTISIG-')).toBeVisible();
    ...
    await expect(page.getByRole('link', { name: 'avatar barukimang.eth Voted' })).toBeVisible();
