...
test('General Test - Check Data Presence Ethereum Sepolia', async ({ page }) => {
    await page.goto(
        '/dao/ethereum-sepolia-0xdf52EFF5F79Fd476Acb255aD195D30D202A071De/dashboard', // ethereum-sepolia DAO using sppPlugin
    );
    // dashboard tab
    await expect(page.getByRole('button', { name: 'ET E2E Test DAO (Ethereum Sepolia)' })).toBeVisible();
    ...
    await expect(page.getByRole('heading', { name: 'Governance (GOV)' })).toBeVisible();
    ...
    await expect(
        page.getByTestId('proposal-details-container').locator('div').filter({ hasText: 'Onchain' }),
    ).toBeVisible();
    ...
    await page.getByRole('button', { name: 'Stage two Stage not reached Stage' }).click();
    ...
    await expect(page.getByRole('link', { name: 'avatar barukimang.eth' })).toBeVisible();
    await expect(page.getByRole('link', { name: 'avatar cgero.eth' })).toBeVisible();
    await expect(page.getByRole('link', { name: 'avatar ea1.aragonx.eth' })).toBeVisible();
    ...
    await expect(page.getByText('Proposal creationAdmin members').first()).toBeVisible();
    await expect(page.getByText('Proposal executionAutomatic').first()).toBeVisible();
    await expect(page.getByRole('heading', { name: 'DAO' })).toBeVisible();
    await expect(page.getByText('NameE2E Test DAO (Ethereum Sepolia)ET')).toBeVisible();
    ...
    await expect(page.getByText('Proposal creationAdmin members').nth(1)).toBeVisible();
    await expect(page.getByText('Proposal executionAutomatic').nth(1)).toBeVisible();
    await expect(page.getByRole('heading', { name: 'Members' })).toBeVisible();
    await page.getByRole('tab', { name: 'Token Voting' }).click();
